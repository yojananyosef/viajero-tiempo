...
## SPEC-012 — Nimrod Corrupto (heraldo de la cima): boss de Q10 que quiere
## culminar la torre para asaltar el cielo. Duerme hasta sellar los 3 pisos;
## al caer cuenta 4/5 (falta romper el ídolo). Patrón del Heraldo de Q07.
...
func _die() -> void:
	...
	if _authoritative() and _quest != null:
		_quest.register_task()  # 4/5: Nimrod caído, falta el ídolo.
	var col := get_node_or_null("Collision") as CollisionShape3D
	if col != null:
...
func _update_label() -> void:
	if _label == null:
		return
	if dead:
		_label.text = "Nimrod derribado"
	elif not awakened:
		_label.text = "Nimrod dormido… sella los 3 pisos"
	else:
		_label.text = "Nimrod Corrupto (heraldo)"
...
func _darken(n: Node) -> void:
	...
	while not stack.is_empty():
		...
		if c is MeshInstance3D:
			...
			m.albedo_color = Color(0.35, 0.12, 0.1, 1)
			m.emission_enabled = true
			m.emission = Color(1.0, 0.35, 0.15, 1) * 0.6
